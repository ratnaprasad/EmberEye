...
class BEMainWindow(QMainWindow):
    ...
    def start_model_training(self):
        ...
        from embereye.core.training_pipeline import YOLOTrainingPipeline
        ...
        # Run precheck before training
        self.training_status_label.setText("Running precheck…")
        self.training_progress.setValue(2)
        
        try:
            pipeline = YOLOTrainingPipeline('./training_data', config)
            precheck_ok, precheck_msg = pipeline.precheck_training()
            
            if not precheck_ok:
                QMessageBox.critical(self, "Training Precheck Failed", precheck_msg)
                self.training_status_label.setText("Precheck failed")
                return
            
            # Show precheck results to user
            QMessageBox.information(self, "Training Precheck", precheck_msg)
        except Exception as e:
            QMessageBox.critical(self, "Precheck Error", f"Error during precheck:\n{e}")
            self.training_status_label.setText("Ready")
            return

        self.start_training_btn.setEnabled(False)
        self.cancel_training_btn.setEnabled(False)
        self.training_status_label.setText("Initializing PyTorch…")
        self.training_progress.setValue(5)
        self.training_just_completed = False  # Reset completion flag for new training

        # Pre-load PyTorch and YOLO in main thread to avoid DLL issues in worker thread
        try:
            import sys
            import torch
            log_server_info(f"GUI Python: {sys.executable}")
            log_server_info(f"PyTorch module: {torch.__file__}")
            _ = torch.rand(1)  # Force PyTorch initialization
            from ultralytics import YOLO
            # Pre-load model to initialize all DLLs
            model_suffix = '-seg' if config.task == 'segment' else ''
            model_name = f'yolov8{config.model_size}{model_suffix}.pt'
            _ = YOLO(model_name)  # Load model to initialize all C++ bindings
            ...
            if "DLL" in error_msg or "c10.dll" in error_msg:
                QMessageBox.critical(
                    self, "PyTorch Error",
                    "PyTorch initialization failed.\n\n"
                    "Fix:\n"
                    "1. Reinstall Visual C++ Redistributable:\n"
                    "   https://aka.ms/vs/17/release/vc_redist.x64.exe\n"
                    "2. Restart Windows\n"
                    "3. Or run: fix_pytorch_windows.ps1"
                )
    ...
    def _on_training_finished(self, ok: bool, msg: str, payload):
        ...
        try:
            ...
            if weights_dir and weights_dir.exists():
                logger.info(f"Creating model version {version} from weights at: {weights_dir}")
                success, msg = manager.create_version(metadata, weights_dir)
                if success:
                    logger.info(f"✓ Version {version} created successfully")
                    self._refresh_model_versions()
                    self._refresh_sandbox_models()  # Update Sandbox dropdown with new version
                    QMessageBox.information(self, "Success", f"Model {version} created successfully!\n\n{msg}")
                else:
                    logger.error(f"Version creation failed: {msg}")
                    QMessageBox.warning(self, "Versioning", f"Training complete, but version creation failed:\n{msg}")
            else:
                logger.error(f"Weights directory not found. best_model_path={best_model_path}, weights_dir={weights_dir}")
    ...
    def show_pfds_add_dialog(self):
        ...
        def on_ok():
            ...
            if not is_valid_ip(ip):
                ...
                return
            try:
                self.emberhawk.add_device(name, ip, loc if loc else None, mode, int(poll))
