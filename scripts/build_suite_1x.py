#!/usr/bin/env python3
"""Build EmberEye Suite 1.x artifacts (Field + Studio) in one command.

This wrapper is intended for stable-line packaging workflows while development
continues on 2.x branches.
"""
...
def build_field(mode: str, build_installer: bool) -> Path:
    env = os.environ.copy()
    # Enable compatibility path if running from 2.x branches that retire
...
def build_studio() -> Path:
    ...
    run([sys.executable, str(ROOT / "embereye-studio" / "build_installer.py")], env=env)
...
def write_manifest(out_dir: Path, field_artifact: Path, studio_artifact: Path, field_mode: str) -> None:
    manifest = {
        "suite": "EmberEye",
        "line": "1.x",
        ...
        "notes": [
            "Build from release/1.1 for stable 1.x outputs.",
            "This wrapper may run in compatibility mode on 2.x branches.",
        ],
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build EmberEye suite for 1.x")
    parser.add_argument(
        "--field-mode",
    ...
    parser.add_argument(
        "--clean",
        action="store_true",
        help="Clean suite-1x output directory before writing artifacts.",
    )
...
def main() -> int:
    ...
    print("=" * 70)
    print("EmberEye Suite 1.x Builder")
    print("=" * 70)
    ...
    write_manifest(out_dir, field_artifact, studio_artifact, args.field_mode)

    print("\n[COMPLETE] Suite 1.x build successful.")
    return 0
