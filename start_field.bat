...
if "%EMBEREYE_FORCE_CPU%"=="1" (
	echo   Launching field app with YOLO model - CPU-only mode...
	start "EmberEye Field" cmd /k "cd /d %~dp0 && set YOLO_MODEL_PATH=%~dp0models\v20260216_213235_model.pt && set EMBEREYE_FORCE_CPU=1 && set CUDA_VISIBLE_DEVICES=-1 && .venv\Scripts\python.exe embereye-field\main.py"
) else (
	echo   Launching field app with YOLO model - GPU auto-detect mode...
	start "EmberEye Field" cmd /k "cd /d %~dp0 && set YOLO_MODEL_PATH=%~dp0models\v20260216_213235_model.pt && set EMBEREYE_FORCE_CPU=0 && .venv\Scripts\python.exe embereye-field\main.py"
)
